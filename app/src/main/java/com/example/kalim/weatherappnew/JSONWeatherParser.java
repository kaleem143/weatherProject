...
import com.example.kalim.weatherappnew.model.DayForecast;
import com.example.kalim.weatherappnew.model.Location;
import com.example.kalim.weatherappnew.model.Weather;
...
public class JSONWeatherParser {
	...
	public static WeatherForecast getForecastWeather(String data) throws JSONException  {
		...
		Log.d("arraylength", String.valueOf(jArr.length()));
		for (int i=0; i < jArr.length(); i++) {
			...
			df.weather.wind.setSpeed((float) jDayForecast.getDouble("speed"));
			df.weather.wind.setDeg((float) jDayForecast.getDouble("deg"));
            df.weather.temperature.setHumadity((float)jDayForecast.getDouble("humidity"));
			// Temp is an object
